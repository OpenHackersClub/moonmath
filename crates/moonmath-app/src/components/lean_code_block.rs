...
/// Renders Lean4 source code with CSS-class-based syntax coloring.
#[component]
pub fn LeanCodeBlock(
    /// Lean4 source code
    code: &'static str,
) -> impl IntoView {
    let highlighted = highlight_lean(code);
    view! {
...
pub fn highlight_lean(code: &str) -> String {
    ...
    while i < len {
        ...
        if chars[i].is_alphanumeric() || chars[i] == '_' {
            ...
            if let Some(c) = cls {
                out.push_str(&format!("<span class=\"{}\">{}</span>", c, escape_html(&word)));
            } else {
...
const KEYWORDS: &[&str] = &[
    "def", "theorem", "lemma", "by", "have", "let", "obtain", "suffices",
    "intro", "notation", "where", "if", "then", "else", "match", "with",
    "fun", "do", "return", "import", "open", "namespace", "end",
    "structure", "class", "instance", "deriving", "section", "variable",
    "example", "noncomputable", "private", "protected", "mutual",
    "inductive", "axiom", "abbrev", "set_option",
];

const TACTICS: &[&str] = &[
    "grind", "simp", "simp_all", "by_cases", "induction", "exact",
    "apply", "rfl", "ring", "omega", "norm_num", "decide", "trivial",
    "constructor", "cases", "rcases", "rintro", "assumption", "contradiction",
    "linarith", "positivity", "field_simp", "push_neg", "use",
];
