...
- **Configuration:** `wrangler.toml` at the repo root pins the Worker name, compatibility date, static assets binding, and the production custom-domain route (`moonmath.openhackers.club` on the `openhackers.club` zone). Secrets (Lean fallback URL, signing key) are set via `wrangler secret put`.
